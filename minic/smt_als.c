/*   在遍历语法树中，完成了符号表的创建、删除等操作，符号表使用的是一个顺序表，实际操作中还可以用hash表等不同形式。注意不同的作用域符号表的开始和结束删除的时机。
    ...
    for (i = 0; i < symbolTable.index; i++)
    {
        if (symbolTable.symbols[i].array)
        {
            printf("%6s %6s %6d %10s %4c %6d\n", symbolTable.symbols[i].name,
                   ...
                   symbolTable.symbols[i].type == INT ? "array_int" : "array_float",
